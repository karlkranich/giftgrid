...
function giftgrid_section_text() {
	echo '<p>Please enter the Cru account number to receive the donations (7-digit account number)';
    echo ' and the URL that givers should be directed to after the Cru checkout process<br />The URL may be left blank ';
    echo 'or could be set to your blog URL.  Example: http://www.kranich.org</p>';
}
...
function giftgrid_setting_URL() {
	$options = get_option('giftgrid_plugin_options');
	echo "<input id='giftgrid_URL' name='giftgrid_plugin_options[URL]' size='20' type='text' value='{$options['URL']}'/>";
}
